...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/last_pulse \
  -H 'Content-Type: application/json' \
  -d '{}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/last_pulse`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
...
```json
{
  "mcpServers": {
    "nist-beacon": {
      "command": "npx",
      "args": ["-y", "@pipeworx/mcp-nist-beacon"]
    }
  }
}
```
...
```bash
npx -y @pipeworx/mcp-nist-beacon
```
