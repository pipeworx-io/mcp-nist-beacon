...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'NIST Randomness Beacon');
}
...
const tools: McpToolExport['tools'] = [
  { name: 'last_pulse', description: 'Fetch the most recent NIST Randomness Beacon pulse: a publicly verifiable, timestamped 512-bit random value signed by NIST, updated every 60 seconds.', inputSchema: { type: 'object', properties: {} } },
  { name: 'pulse_at', description: 'Fetch the NIST Randomness Beacon pulse closest to a given Unix timestamp in milliseconds (time_ms); returns the 512-bit output value and chain/signature metadata.', inputSchema: { type: 'object', properties: { time_ms: { type: 'number' } }, required: ['time_ms'] } },
  {
...
async function callTool(name: string, args: Record<string, unknown>): Promise<unknown> {
  const get = async (path: string) => {
    ...
    if (!res.ok) throw await httpError(res, 'NIST Beacon');
    return res.json();
  };
